...
def create_clients_mnist_and_mnist_rotation_90(client_solver):
    ...
    for i in range(num_clients):
        ...
        t_mnist_ds = MnistLocal(mnist_sampler.sample(class_vec=range(10), num_samples=50))
        t_mnist_rotation_ds = MnistRotationLocal90(
            mnist_rotation_sampler.sample(class_vec=range(10), num_samples=50))
        client = Client(ID=i, ds=ConcatDataset([mnist_ds, mnist_rotation_ds]),test_ds=ConcatDataset([t_mnist_ds, t_mnist_rotation_ds]), solver=client_solver, tag=tag)
...
def create_clients_cifar_and_cifar_rotation_90(client_solver):
    ...
    for i in range(num_clients):
        ...
        test_cifar_ds = CifarLocal(cifar_sampler.sample(class_vec=range(10), num_samples=50))
        test_cifar_rotation_ds = CifarRotationLocal90(
            cifar_rotation_sampler.sample(class_vec=range(10), num_samples=50))
        client = Client(ID=i, ds=ConcatDataset([cifar_ds, cifar_rotation_ds]),test_ds=ConcatDataset([test_cifar_ds, test_cifar_rotation_ds]), solver=client_solver, tag=tag)
        client_vec.append(client)
...
def create_clients_mnist_rotation_4set(client_solver):
    ...
    for i in range(num_clients):
        ...
        test_mnist0_ds = MnistLocal(mnist_sampler.sample(class_vec=range(10), num_samples=50))
        test_mnist90_ds = MnistRotationLocal90(
            mnist_rotation_sampler.sample(class_vec=range(10), num_samples=50))
        test_mnist180_ds = MnistRotationLocal180(
            mnist_rotation_sampler.sample(class_vec=range(10), num_samples=50))
        test_mnist270_ds = MnistRotationLocal270(
            mnist_rotation_sampler.sample(class_vec=range(10), num_samples=50))

        client = Client(ID=i, ds=ConcatDataset([mnist0_ds, mnist90_ds, mnist180_ds, mnist270_ds]),test_ds=ConcatDataset([test_mnist0_ds, test_mnist90_ds, test_mnist180_ds, test_mnist270_ds]), solver=client_solver,
                        tag=tag)
        client_vec.append(client)
...
def create_clients_letters_lower_and_upper(client_solver):
    ...
    for i in range(num_clients):
        ...
        test_lower_ds = LettersLowerLocal(lower_sampler.sample(class_vec=range(26), num_samples=num_samples_lower))
        test_upper_ds = LettersUpperLocal(upper_sampler.sample(class_vec=range(26), num_samples=num_samples_upper))
        client = Client(ID=i, ds=ConcatDataset([lower_ds, upper_ds]),test_ds=ConcatDataset([test_lower_ds, test_upper_ds]), solver=client_solver, tag=tag)
        client_vec.append(client)
...
def create_clients_letters_rotation_8set(client_solver):
    ...
    for k in range(num_clients):
        ...
        test_client_lower0_ds = LettersLowerLocal(lower_sampler.sample(range(26), num_samples=num_samples_vec[0]))
        test_client_lower90_ds = LettersLowerLocal90(lower_sampler.sample(range(26), num_samples=num_samples_vec[1]))
        test_client_lower180_ds = LettersLowerLocal180(lower_sampler.sample(range(26), num_samples=num_samples_vec[2]))
        test_client_lower270_ds = LettersLowerLocal270(lower_sampler.sample(range(26), num_samples=num_samples_vec[3]))
        test_client_upper0_ds = LettersUpperLocal(upper_sampler.sample(range(26), num_samples=num_samples_vec[4]))
        test_client_upper90_ds = LettersUpperLocal90(upper_sampler.sample(range(26), num_samples=num_samples_vec[5]))
        test_client_upper180_ds = LettersUpperLocal180(upper_sampler.sample(range(26), num_samples=num_samples_vec[6]))
        test_client_upper270_ds = LettersUpperLocal270(upper_sampler.sample(range(26), num_samples=num_samples_vec[7]))
        test_client_ds_vec = [test_client_lower0_ds, test_client_lower90_ds, test_client_lower180_ds,test_client_lower270_ds, test_client_upper0_ds, test_client_upper90_ds, test_client_upper180_ds, test_client_upper270_ds]

        client = Client(ID=k, ds=ConcatDataset(ds_vec), test_ds=ConcatDataset(test_client_ds_vec),solver=client_solver, tag=tag)
        client_vec.append(client)
...
def create_clients_letters_rotation_4set(client_solver):
    ...
    for k in range(num_clients):
        ...
        test_client_lower0_ds = LettersLowerLocal(lower_sampler.sample(range(26), num_samples=num_samples_vec[0]))
        test_client_lower90_ds = LettersLowerLocal90(lower_sampler.sample(range(26), num_samples=num_samples_vec[1]))
        test_client_upper0_ds = LettersUpperLocal(upper_sampler.sample(range(26), num_samples=num_samples_vec[2]))
        test_client_upper90_ds = LettersUpperLocal90(upper_sampler.sample(range(26), num_samples=num_samples_vec[3]))
        test_client_ds_vec=[test_client_lower0_ds, test_client_lower90_ds, test_client_upper0_ds, test_client_upper90_ds]
...
def create_clients_lr2(client_solver):
    ...
    for i in range(num_clients):
        ...
        t_lr_ds1 = LR2ALocal(sampler.sample(num_samples_1))
        t_lr_ds2 = LR2BLocal(sampler.sample(num_samples_2))
        ...
        client = Client(ID=i, ds=ConcatDataset([lr_ds1, lr_ds2]),test_ds=ConcatDataset(t_lr_ds1,t_lr_ds2), solver=client_solver, tag=tag)
        client_vec.append(client)
...
def create_clients_lr2_div(client_solver, weight_var):
    ...
    for k in range(num_clients):
        ...
        lr_ds_vec = [LRLocal(weight=weight_vec[s], noise_var=noise_var, size=num_samples_vec[s]) for s in
                     range(mixture_size)]
        t_lr_ds_vec = [LRLocal(weight=weight_vec[s], noise_var=noise_var, size=num_samples_vec[s]) for s in
                     range(mixture_size)]
        tag = 'Mixture of {} set lr10 data with respectively {} samples, total {} samples.'.format(
            mixture_size, num_samples_vec, num_samples)
        client = Client(ID=k, ds=ConcatDataset(lr_ds_vec),test_ds=ConcatDataset(t_lr_ds_vec), solver=client_solver, tag=tag)
        client_vec.append(client)
        num_samples_vec.append(num_samples)
        mixture.append(num_samples_vec)
...
def create_clients_digits5_5set(client_solver):
    from datawrappers.generate_Digit5 import digit5_dataset_read
    num_clients = 100
    num_samples_min = 200
    num_samples_max = 301
    ...
    for d in domains:
        ...
        for i in range(20):
            num_samples = np.random.randint(num_samples_min, num_samples_max)
            num_samples=300
            tag = 'Digits-five. {} full {}.'.format(num_samples,d)
            client = Client(ID=i+posi, ds=torch.utils.data.DataLoader(train_loader.dataset,
                                                                          batch_size=client_solver.batch_size,
                                                                          sampler=torch.utils.data.sampler.SubsetRandomSampler(
                                                                              list(random.sample(range(25000), num_samples)))),
                            test_ds=torch.utils.data.DataLoader(train_loader.dataset,
                                                                          batch_size=client_solver.batch_size,
                                                                          sampler=torch.utils.data.sampler.SubsetRandomSampler(
                                                                              list(random.sample(range(25000), 100)))),
                            solver=client_solver,
                            tag=tag)
            client_vec.append(client)
        posi = posi + 20
        test_dataloader.append(torch.utils.data.DataLoader(test_loader.dataset,
                                    batch_size=100,
                                    sampler=torch.utils.data.sampler.SubsetRandomSampler(
                                        list(random.sample(range(len(test_loader.dataset)), 500)))))
    param_dict = {
        'ds_description': '(4) Full mnist rotations -0 -90 -180 -270, sizes allocated randomly',
        'num_clusters': 4,
        'num_classes': 10,
        ...
        'num_samples_min': num_samples_min,
        'num_samples_max': num_samples_max,
        'num_samples_test_each': len(test_dataloader[0]),
        'client_tags': {}
    }
    for client in client_vec:
        param_dict['client_tags']['client_' + str(client.ID)] = client.tag

    return client_vec, test_dataloader, param_dict