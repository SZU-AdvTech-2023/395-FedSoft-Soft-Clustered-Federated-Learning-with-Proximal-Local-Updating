...
class Client:
    def __init__(self, ID, ds,test_ds, solver, tag=''):
        self.ID = ID
        self.tag = tag

        self.ds = ds
        self.solver = solver
        if 'Digits-five' in tag:
            self.num_samples = len(ds.sampler)
            self.loader = ds
            ...
            self.test_loader = DataLoader(test_ds, batch_size=self.solver.batch_size)
