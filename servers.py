...
class Server:
    ...
    def _run_fedsoft(self, num_global_epochs):
        ...
        for t in range(num_global_epochs):
            ...
            print('time for importance estimation is {}'.format(time.time()-start_time))
            start_time = time.time()
            ...
            print('time for local updates is {}'.format(time.time()-start_time))
            # Aggregation
            start_time=time.time()
            self._aggregate_fedsoft(selection)
            print('time for epoch {} is {}'.format(t, time.time()-start_time))
            ...
            if t%5 ==0:
                ...
                print('time for validation is {}'.format(time.time()-start_time))
